...
def test_wochenplan_aus_kochbuch_uebernimmt_zubereitung(monkeypatch, tmp_path):
    # Ein Kochbuch-Rezept mit gespeicherter Zubereitung soll die auch in den
    # zusammengestellten Wochenplan mitbringen, nicht nur Titel + Zutaten.
    monkeypatch.setenv("KOCHBUCH_DIR", str(tmp_path / "rezepte"))
    from app.tools import kochbuch
...
def test_wochenplan_loeschen(monkeypatch, tmp_path):
    ...
    geloescht = client.delete(f"/wochenplan/{gespeichert['id']}")
    assert geloescht.status_code == 200
    assert client.get("/wochenplan").json() == []


def test_wochenplan_loeschen_unbekannte_id_gibt_404(monkeypatch, tmp_path):
    monkeypatch.setenv("WOCHENPLAENE_DIR", str(tmp_path))
