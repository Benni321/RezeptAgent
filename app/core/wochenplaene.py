...
def speichere(nachricht: str, antwort: str, gerichte: list[dict], titel: str = "",
             anzahl_angefragt: int | None = None) -> dict:
    ...
    anzahl_angefragt: urspruenglich gewuenschte Gerichte-/Tage-Anzahl (kann von
    len(gerichte) abweichen, wenn nicht alle gewuenschten Gerichte geplant werden
    konnten -- z. B. bei knappem Groq-Free-Tier-Budget waehrend eines langen
    Wochenplan-Laufs). Ohne Angabe (z. B. beim deterministischen Kochbuch-Weg,
    wo der Nutzer exakt so viele Gerichte auswaehlt, wie er will) = len(gerichte).
    """
    verzeichnis = _wochenplaene_dir()
    ...
    daten = {
        ...
        "anzahl_angefragt": anzahl_angefragt if anzahl_angefragt is not None else len(gerichte),
    }
