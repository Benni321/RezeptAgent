...
class ChatAntwort(BaseModel):
    ...
    gerichte: Optional[list[dict]] = None  # NUR bei Wochenplan-Antworten gesetzt (je {"titel", "zutaten", "zubereitung"})
    anzahl_angefragt: Optional[int] = None  # urspruenglich gewuenschte Gerichte-Anzahl (Wochenplan; kann > len(gerichte) sein)
...
class WochenplanSpeichern(BaseModel):
    ...
    anzahl_angefragt: Optional[int] = Field(default=None, ge=1)
